...
pub fn diff_spans(diff: &str, token: &str) -> Vec<(Option<Color>, SpanLine)> {
    use picocode_core::transcript::DiffRow;

    let (rows, old_src, new_src) = picocode_core::transcript::parse_diff(diff);
    let old_hl = highlight(&old_src, token);
    let new_hl = highlight(&new_src, token);
    ...
    rows.into_iter()
        .map(|row| match row {
            ...
            DiffRow::Other(l) => (None, vec![(Style::new().fg(Color::DarkGray), l)]),
        })
        .collect()
}
